...
func TestRewards(t *testing.T) {
	...
	t.Run("GetByParty", func(t *testing.T) {
		...
		actual, _, err := rs.Get(context.Background(), &party1ID, nil, nil)
		require.NoError(t, err)
		assertRewardsMatch(t, expected, actual)
	})
	...
	t.Run("GetByAsset", func(t *testing.T) {
		...
		actual, _, err := rs.Get(context.Background(), nil, &asset1ID, nil)
		require.NoError(t, err)
		assertRewardsMatch(t, expected, actual)
	})
	...
	t.Run("GetByAssetAndParty", func(t *testing.T) {
		...
		actual, _, err := rs.Get(context.Background(), &party1ID, &asset1ID, nil)
		require.NoError(t, err)
		assertRewardsMatch(t, expected, actual)
	})
	...
	t.Run("GetPagination", func(t *testing.T) {
		...
		actual, _, err := rs.Get(context.Background(), nil, nil, &p)
		require.NoError(t, err)
...
func testRewardsCursorPaginationNoPagination(t *testing.T) {
	...
	rewards, pageInfo, err := rs.Get(ctx, &partyID, &assetID, pagination)
	...
	assert.Equal(t, entities.NewCursor("637").Encode(), pageInfo.StartCursor)
	assert.Equal(t, entities.NewCursor("1027").Encode(), pageInfo.EndCursor)
...
func testRewardsCursorPaginationFirstPage(t *testing.T) {
	...
	assert.Equal(t, entities.NewCursor("637").Encode(), pageInfo.StartCursor)
	assert.Equal(t, entities.NewCursor("643").Encode(), pageInfo.EndCursor)
	assert.False(t, pageInfo.HasPreviousPage)
...
func testRewardsCursorPaginationLastPage(t *testing.T) {
	...
	assert.Equal(t, int64(1027), rewards[len(rewards)-1].EpochID)
	assert.Equal(t, entities.NewCursor("757").Encode(), pageInfo.StartCursor)
	assert.Equal(t, entities.NewCursor("1027").Encode(), pageInfo.EndCursor)
...
func testRewardsCursorPaginationFirstPageAfter(t *testing.T) {
	...
	populateTestRewards(ctx, t, bs, ps, as, rs)
	first := int32(3)
	after := entities.NewCursor("643").Encode()
	pagination, err := entities.CursorPaginationFromProto(&v2.Pagination{
		First: &first,
		After: &after,
	})
	...
	assert.Equal(t, entities.NewCursor("737").Encode(), pageInfo.StartCursor)
	assert.Equal(t, entities.NewCursor("744").Encode(), pageInfo.EndCursor)
	assert.True(t, pageInfo.HasPreviousPage)
	assert.True(t, pageInfo.HasNextPage)
}
...
func testRewardsCursorPaginationLastPageBefore(t *testing.T) {
	...
	populateTestRewards(ctx, t, bs, ps, as, rs)
	last := int32(3)
	before := entities.NewCursor("757").Encode()
	pagination, err := entities.CursorPaginationFromProto(&v2.Pagination{
		Last:   &last,
		Before: &before,
	})
	...
	assert.Equal(t, entities.NewCursor("741").Encode(), pageInfo.StartCursor)
	assert.Equal(t, entities.NewCursor("747").Encode(), pageInfo.EndCursor)
	assert.True(t, pageInfo.HasPreviousPage)
	assert.True(t, pageInfo.HasNextPage)
}
