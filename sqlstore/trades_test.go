...
func testTradesCursorPaginationByMarketNoCursor(t *testing.T) {
	...
	marketID := "deadbeef"
	results := ts.GetByMarketWithCursor(ctx, marketID, pagination)
	require.NoError(t, err)

	assert.Len(t, results.Entities, 4)
	assert.Equal(t, int64(4), results.TotalCount)
	assert.Equal(t, "02a16077", results.Entities[0].ID.String())
	assert.Equal(t, uint64(1), results.Entities[0].Size)
	assert.Equal(t, "7a797e0e", results.Entities[3].ID.String())
	assert.Equal(t, uint64(4), results.Entities[3].Size)
	assert.False(t, results.PageInfo.HasNextPage)
	assert.False(t, results.PageInfo.HasPreviousPage)
	assert.Equal(t, "02a16077", results.Entities[0].ID.String())
	assert.Equal(t, "7a797e0e", results.Entities[3].ID.String())
}
...
func testTradesCursorPaginationByPartyNoMarketNoCursor(t *testing.T) {
	...
	partyID := "dabbad00"
	results := ts.GetByPartyWithCursor(ctx, partyID, nil, pagination)
	require.NoError(t, err)
	assert.Equal(t, int64(5), results.TotalCount)
	assert.Len(t, results.Entities, 5)

	assert.Equal(t, "02a16077", results.Entities[0].ID.String())
	assert.Equal(t, uint64(1), results.Entities[0].Size)
	assert.Equal(t, "7bb2356e", results.Entities[4].ID.String())
	assert.Equal(t, uint64(5), results.Entities[4].Size)
...
func testTradesCursorPaginationByPartyAndMarketNoCursor(t *testing.T) {
	...
	partyID := "dabbad00"
	marketID := "deadbeef"
	results := ts.GetByPartyWithCursor(ctx, partyID, &marketID, pagination)
	require.NoError(t, err)
	assert.Equal(t, int64(4), results.TotalCount)
	assert.Len(t, results.Entities, 4)
	assert.Equal(t, "02a16077", results.Entities[0].ID.String())
...
func testTradesCursorPaginationByMarketWithCursorFirst(t *testing.T) {
	...
	assert.Equal(t, int64(4), results.TotalCount)
	assert.Len(t, results.Entities, 2)
	assert.Equal(t, "02a16077", results.Entities[0].ID.String())
	assert.Equal(t, uint64(1), results.Entities[0].Size)
	assert.Equal(t, "44eea1bc", results.Entities[1].ID.String())
	assert.Equal(t, uint64(2), results.Entities[1].Size)
	assert.True(t, results.PageInfo.HasNextPage)
	assert.False(t, results.PageInfo.HasPreviousPage)
	assert.Equal(t, "02a16077", results.Entities[0].ID.String())
	assert.Equal(t, "44eea1bc", results.Entities[1].ID.String())
}
...
func testTradesCursorPaginationByMarketWithCursorLast(t *testing.T) {
	...
	assert.Equal(t, int64(4), results.TotalCount)
	assert.Len(t, results.Entities, 2)
	assert.Equal(t, "65be62cd", results.Entities[0].ID.String())
	assert.Equal(t, uint64(3), results.Entities[0].Size)
	assert.Equal(t, "7a797e0e", results.Entities[1].ID.String())
	assert.Equal(t, uint64(4), results.Entities[1].Size)
	assert.False(t, results.PageInfo.HasNextPage)
	assert.True(t, results.PageInfo.HasPreviousPage)
	assert.Equal(t, "65be62cd", results.Entities[0].ID.String())
	assert.Equal(t, "7a797e0e", results.Entities[1].ID.String())
}
...
func testTradesCursorPaginationByPartyWithCursorNoMarketLast(t *testing.T) {
	...
	assert.Equal(t, int64(5), results.TotalCount)
	assert.Len(t, results.Entities, 2)
	assert.Equal(t, "7a797e0e", results.Entities[0].ID.String())
	assert.Equal(t, uint64(4), results.Entities[0].Size)
	assert.Equal(t, "7bb2356e", results.Entities[1].ID.String())
	assert.Equal(t, uint64(5), results.Entities[1].Size)
	assert.False(t, results.PageInfo.HasNextPage)
	assert.True(t, results.PageInfo.HasPreviousPage)
	assert.Equal(t, "7a797e0e", results.Entities[0].ID.String())
	assert.Equal(t, "7bb2356e", results.Entities[1].ID.String())
...
func testTradesCursorPaginationByMarketWithCursorForward(t *testing.T) {
	...
	assert.Equal(t, int64(4), results.TotalCount)
	assert.Len(t, results.Entities, 2)
	assert.Equal(t, "44eea1bc", results.Entities[0].ID.String())
	assert.Equal(t, uint64(2), results.Entities[0].Size)
	assert.Equal(t, "65be62cd", results.Entities[1].ID.String())
	assert.Equal(t, uint64(3), results.Entities[1].Size)
	assert.True(t, results.PageInfo.HasNextPage)
	assert.True(t, results.PageInfo.HasPreviousPage)
	assert.Equal(t, "44eea1bc", results.Entities[0].ID.String())
	assert.Equal(t, "65be62cd", results.Entities[1].ID.String())
...
func testTradesCursorPaginationByMarketNoCursorNewestFirst(t *testing.T) {
	...
	assert.Equal(t, "7a797e0e", results.Entities[0].ID.String())
	assert.Equal(t, uint64(4), results.Entities[0].Size)
	assert.Equal(t, "02a16077", results.Entities[3].ID.String())
	assert.Equal(t, uint64(1), results.Entities[3].Size)
	assert.False(t, results.PageInfo.HasNextPage)
	assert.False(t, results.PageInfo.HasPreviousPage)
	assert.Equal(t, "7a797e0e", results.Entities[0].ID.String())
	assert.Equal(t, "02a16077", results.Entities[3].ID.String())
...
func testTradesCursorPaginationByPartyNoMarketNoCursorNewestFirst(t *testing.T) {
	...
	assert.Equal(t, "7bb2356e", results.Entities[0].ID.String())
	assert.Equal(t, uint64(5), results.Entities[0].Size)
	assert.Equal(t, "02a16077", results.Entities[4].ID.String())
	assert.Equal(t, uint64(1), results.Entities[4].Size)
...
func testTradesCursorPaginationByMarketWithCursorLastNewestFirst(t *testing.T) {
	...
	assert.Equal(t, "44eea1bc", results.Entities[0].ID.String())
	assert.Equal(t, uint64(2), results.Entities[0].Size)
	assert.Equal(t, "02a16077", results.Entities[1].ID.String())
	assert.Equal(t, uint64(1), results.Entities[1].Size)
	assert.False(t, results.PageInfo.HasNextPage)
	assert.True(t, results.PageInfo.HasPreviousPage)
	assert.Equal(t, "44eea1bc", results.Entities[0].ID.String())
	assert.Equal(t, "02a16077", results.Entities[1].ID.String())
